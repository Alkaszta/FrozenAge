...
namespace FrozenAge.Projectiles
{
	public class HydraTalonsArrow : ModProjectile 
	{
		...
		{
			int DustID = Dust.NewDust(new Vector2(projectile.position.X, projectile.position.Y + 2f), projectile.width + 4, projectile.height + 4, 15, projectile.velocity.X * 0.2f, projectile.velocity.Y * 0.2f, 15, default(Color), 0.75f);
		...
		public override void OnHitNPC(NPC target, int damage, float knockback, bool crit)
		{
			...
			if (projectilecenter >= -projectile.owner)
			{
				...
				int Proj = Projectile.NewProjectile(target.position.X-projectilepositionX, target.position.Y+projectilepositionY, speedX, speedY, ModContent.ProjectileType<HydraTalons>(), projectile.damage * 2, projectileknockBack, projectile.owner, 0f, 0f);
			}
